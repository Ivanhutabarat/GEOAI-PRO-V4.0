...
/**
 * 44. Barton's Q-System Tunnel & Wellbore Stability
 * Q = (RQD / Jn) * (Jr / Ja) * (Jw / SRF)
 */
export function calculateBartonQSystem(rqd: number, jn: number, jr: number, ja: number, jw: number, srf: number): OmniScienceResult {
  const rqdClamped = clamp(rqd, 3, 100);
  const jnClamped = Math.max(0.1, jn);
  const jaClamped = Math.max(0.1, ja);
  const srfClamped = Math.max(0.1, srf);
  
  const qValue = (rqdClamped / jnClamped) * (jr / jaClamped) * (jw / srfClamped);
  
  let classification = "Exceptionally Good Rock (Class I - No support required)";
  let hazardStatus: OmniScienceResult['hazardStatus'] = 'SAFE';

  if (qValue < 0.01) {
    classification = "Exceptionally Poor Rock (Class VIII - Extremely heavy squeezing, instant collapsing)";
    hazardStatus = 'CRITICAL';
  } else if (qValue < 0.1) {
    classification = "Extremely Poor Rock (Class VII - Concrete lining & heavy steel arches)";
    hazardStatus = 'CRITICAL';
  } else if (qValue < 1.0) {
    classification = "Poor Rock (Class VI - Dynamic rock bolting & fiber shotcrete)";
    hazardStatus = 'WARNING';
  } else if (qValue < 4.0) {
    classification = "Fair Rock (Class V - Systematic pattern rock bolts)";
    hazardStatus = 'HEED';
  } else if (qValue < 10.0) {
    classification = "Medium Good Rock (Class IV)";
  } else if (qValue < 40.0) {
    classification = "Good Rock (Class III)";
  } else if (qValue < 100.0) {
    classification = "Very Good Rock (Class II)";
  }

  return {
    calculatedValue: Number(qValue.toFixed(4)),
    unit: "Barton Q-Index Value",
    classification,
    hazardStatus,
    dynamicExplanation: `Barton Q-System rock quality index calculates to ${qValue.toFixed(4)}. Evaluated geological layer behaves as '${classification}'. Support systems must adapt.`
  };
}

/**
 * 45. Earthquake Peak Ground Acceleration (PGA) Attenuation
 * PGA = (1230 * e^(0.8 * M)) / (D + 40)^1.6 / 981.0 (in g-forces)
 */
export function calculateEarthquakePGA(magnitude: number, distanceKm: number): OmniScienceResult {
  const M = clamp(magnitude, 1, 10);
  const D = clamp(distanceKm, 0.1, 1000);
  
  // Esteva standard equation in cm/s^2 (Gal)
  const pgaGal = (1230 * Math.exp(0.8 * M)) / Math.pow(D + 40, 1.6);
  // Convert Gal to g-force (1g ≈ 981.0 cm/s^2)
  const pgaG = pgaGal / 981.0;

  let classification = "Instrumental / Weak Shaking (MMI I-III)";
  let hazardStatus: OmniScienceResult['hazardStatus'] = 'SAFE';

  if (pgaG >= 0.60) {
    classification = "Extreme Catastrophic Shaking, Severe Structural Destruction (MMI IX-X+)";
    hazardStatus = 'CRITICAL';
  } else if (pgaG >= 0.35) {
    classification = "Violent Shaking, Moderate to Heavy Damage (MMI VIII)";
    hazardStatus = 'CRITICAL';
  } else if (pgaG >= 0.18) {
    classification = "Very Strong Shaking, Moderate Damage (MMI VII)";
    hazardStatus = 'WARNING';
  } else if (pgaG >= 0.08) {
    classification = "Strong Shaking, Light Damage (MMI VI)";
    hazardStatus = 'HEED';
  } else if (pgaG >= 0.03) {
    classification = "Moderate Shaking, No Damage (MMI IV-V)";
    hazardStatus = 'HEED';
  }

  return {
    calculatedValue: Number(pgaG.toFixed(4)),
    unit: "Peak Ground Acceleration (g-force)",
    classification,
    hazardStatus,
    dynamicExplanation: `Predictive attenuation model logs an expected PGA of ${pgaG.toFixed(3)}g at ${D.toFixed(1)} km from epicenter for a Magnitude ${M.toFixed(1)} event. Seismic waves represent a '${hazardStatus}' threat.`
  };
}

/**
 * 46. Darcy Radial Reservoir Inflow (Dupuit-Thiem Model)
 * Q = (2 * PI * k * H * drawdown) / ln(R_influence / r_well)
 */
export function calculateDarcyRadialInflow(k: number, H: number, drawdown: number, radiusRatio: number): OmniScienceResult {
  const kClamped = Math.max(0.001, k);
  const HClamped = Math.max(0.1, H);
  const drawdownClamped = Math.max(0.01, drawdown);
  const ratioClamped = Math.max(1.1, radiusRatio);

  const qFlow = (2 * Math.PI * kClamped * HClamped * drawdownClamped) / Math.log(ratioClamped);

  let classification = "Ultra-Low Flow (Hydraulically locked/dense shale matrix)";
  let hazardStatus: OmniScienceResult['hazardStatus'] = 'HEED';

  if (qFlow > 2500) {
    classification = "Torrential Aqueous Inflow (Well Flooding / Aquifer Blowout risk)";
    hazardStatus = 'CRITICAL';
  } else if (qFlow > 500) {
    classification = "High Extraction Inflow (Productive sand layers)";
    hazardStatus = 'WARNING';
  } else if (qFlow > 100) {
    classification = "Moderate Stable Yield";
    hazardStatus = 'SAFE';
  } else if (qFlow > 10) {
    classification = "Low Productive Inflow Rate (Consider hydraulic fracking)";
    hazardStatus = 'SAFE';
  }

  return {
    calculatedValue: Number(qFlow.toFixed(3)),
    unit: "cubic meters per day (m³/day)",
    classification,
    hazardStatus,
    dynamicExplanation: `Radial borehole pressure drop yields ${qFlow.toFixed(1)} m³/day inflow. Permeability is ${k.toFixed(2)} m/day over ${H.toFixed(1)}m thick sand formation.`
  };
}
