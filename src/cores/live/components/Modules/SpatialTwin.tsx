...
export default function SpatialTwin() {
  ...
  const triggerTectonicSlip = useGeoDataStore(state => state.triggerTectonicSlip);
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              onClick={() => {
                triggerTectonicSlip();
                // Synthesize browser low-frequency seismic hum audio
                try {
                  ...
                  osc.frequency.linearRampToValueAtTime(10, ctx.currentTime + 1.8);
              ...
              className={`text-[9px] px-2.5 py-0.5 rounded font-mono font-bold border transition-all cursor-pointer ${
                isTremorActive 
                  ? 'bg-orange-500/30 text-orange-400 border-orange-500/70 animate-pulse' 
                  : 'bg-orange-500/10 text-orange-400 border-orange-500/40 hover:bg-orange-500/20'
              }`}
            >
              {isTremorActive ? '🌋 SLIP ACTIVE' : '☄ TRIGGER SLIP'}
            </button>
          </div>
          <div className="h-6 w-px bg-[#333]"></div>
