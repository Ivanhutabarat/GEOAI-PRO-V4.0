...
function SeismicShockwave({ planes }: { planes: THREE.Plane[] }) {
  const isTremorActive = useGeoDataStore(state => state.isTremorActive);
  const tremorIntensity = useGeoDataStore(state => state.tremorIntensity);
  const faultPositionX = useGeoDataStore(state => state.faultPositionX);

  if (!isTremorActive || tremorIntensity <= 0) return null;

  // Concentric expanding shockwave circles
  const radius = (1.1 - tremorIntensity) * 15;

  return (
    <group position={[faultPositionX, 0, 0]}>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.05, 0]}>
        <ringGeometry args={[radius * 0.94, radius, 32]} />
        <meshBasicMaterial color="#ff5722" transparent opacity={tremorIntensity * 0.75} side={THREE.DoubleSide} clippingPlanes={planes} />
      </mesh>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.05, 0]}>
        <ringGeometry args={[radius * 0.44, radius * 0.5, 32]} />
        <meshBasicMaterial color="#ff5722" transparent opacity={tremorIntensity * 0.45} side={THREE.DoubleSide} clippingPlanes={planes} />
      </mesh>
    </group>
  );
}
...
function EarthGeologicalBlockModel({ sliceZ, planes }: EarthGeologicalBlockModelProps) {
  ...
  useFrame(({ clock }) => {
    ...
    if (isTremorActive && tremorIntensity > 0) {
      const time = clock.getElapsedTime();
      const jitterAmp = tremorIntensity * 0.25;
      groupRef.current.position.x = Math.sin(time * 85) * jitterAmp;
      groupRef.current.position.y = Math.cos(time * 95) * jitterAmp;
      groupRef.current.position.z = Math.sin(time * 70) * jitterAmp;
    } else {
...
function GeologicalModel({ sliceZ, activePayload }: { sliceZ: number, activePayload: any }) {
  ...
  return (
    <group>
      ...
      {/* Seismic propagating shockwaves radiating from slip epicentre */}
      <SeismicShockwave planes={planes} />

      {/* Red vertical glowing fault plane with pulse glowing active strain */}
      {faultActive && (
        <mesh position={[faultPositionX, 0, 0]} rotation={[0, Math.PI / 2, 0]}>
          <planeGeometry args={[16, 8]} />
          <meshStandardMaterial 
            color={isTremorActive ? "#ff3300" : "#ef4444"} 
            transparent 
            opacity={isTremorActive ? 0.35 + tremorIntensity * 0.35 : 0.15} 
            side={THREE.DoubleSide} 
            roughness={0.1}
            metalness={0.9}
            emissive="#ef4444"
            emissiveIntensity={isTremorActive ? 0.3 + tremorIntensity * 4.0 : 0.3}
            clippingPlanes={planes}
          />
        </mesh>
      )}
...
export const WebGPUDetector = () => {
  const [hasWebGPU, setHasWebGPU] = React.useState(false);
  React.useEffect(() => {
    // Check WebGPU Support (navigator.gpu)
    if ('gpu' in navigator) {
      setHasWebGPU(true);
    }
  }, []);
  ...
  return (
    <div className="absolute top-2 left-2 z-[200] font-mono text-[8px] bg-black/50 px-2 py-1 border rounded border-gray-800 flex items-center gap-2 pointer-events-none">
      <div className={`w-1.5 h-1.5 rounded-full ${hasWebGPU ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
      <span className={hasWebGPU ? 'text-emerald-400' : 'text-red-400'}>
        {hasWebGPU ? 'WEBGPU ACCELERATION (ACTIVE) - 3D EXTREME SIMULATION' : 'WEBGL FALLBACK - STANDARD RENDERING'}
      </span>
      {hasWebGPU && <span className="text-gray-500 uppercase tracking-widest ml-1 border-l border-gray-700 pl-1">LoD CHUNKING ON</span>}
    </div>
  );
};
...
export default function SpatialTwinEngine({ sliceZ, activePayload, eventSource }: { sliceZ: number, activePayload: any, eventSource: HTMLDivElement | null }) {
  const points = useGeoDataStore(state => state.points);
  const { performanceMode, themeMode, fps } = useAppContext();
  
  React.useEffect(() => {
    if ('gpu' in navigator) {
        console.log("[SpatialTwinEngine] WebGPU active: streaming high-res 100k+ LoD Point Cloud.");
    }
  }, []);

  const isEco = performanceMode === 'ECO_PERFORMANCE';
  const isLight = themeMode === 'LIGHT';
  const bgColor = isLight ? '#f1f5f9' : '#070709';
  const gridCellColor = isLight ? '#cbd5e1' : '#222225';
  const gridSecColor = isLight ? '#94a3b8' : '#333338';
  ...
  return (
    <SceneErrorBoundary>
      ...
      <div className="absolute inset-0 w-full h-full min-h-0">
        ...
        {/* Dynamic Performance & Profiler Floating Indicator */}
        <div className="absolute top-2 right-2 z-[200] font-mono text-[8px] bg-black/70 backdrop-blur-xs px-2.5 py-1 border rounded-lg border-gray-800 flex items-center gap-2 pointer-events-none select-none">
          <span className={`w-1.5 h-1.5 rounded-full ${isEco ? 'bg-amber-400' : 'bg-cyan-400 animate-pulse'}`} />
          <span className={isEco ? 'text-amber-300 font-bold' : 'text-cyan-300 font-bold'}>
            {isEco ? 'ECO MODE (1.0x DPR • 60 FPS LOCK)' : 'HI-FIDELITY (MULTI-SAMPLE AA • VOLUMETRIC)'}
          </span>
        ...
        >
          ...
          <fog attach="fog" args={[bgColor, 15, isEco ? 40 : 55]} />
          <ambientLight intensity={isLight ? 0.7 : 0.4} />
          <directionalLight position={[10, 20, 10]} intensity={isLight ? 1.8 : 1.5} color="#fff1e6" />
          <pointLight position={[-15, -5, -15]} intensity={isLight ? 1.2 : 1.0} color="#b8c5e6" />
          <Center>
            <group scale={[1.4, 1.4, 1.4]}>
              <GeologicalModel sliceZ={sliceZ} activePayload={activePayload} />
            </group>
          </Center>
          <Grid infiniteGrid fadeDistance={isEco ? 30 : 45} fadeStrength={5} cellColor={gridCellColor} sectionColor={gridSecColor} position={[0, -5, 0]} />
