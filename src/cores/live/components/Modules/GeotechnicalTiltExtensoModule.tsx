...
import { ResponsiveContainer, ScatterChart, Scatter, XAxis, YAxis, ZAxis, Tooltip, CartesianGrid, Cell, Legend } from 'recharts';
...
export default function GeotechnicalTiltExtensoModule() {
  ...
  useEffect(() => {
    ...
    // REMOVED MOCK UPDATE
  }, [activeFileName, tilt, strain, inferredTilt, inferredStrain]);
          ...
          <div className="h-56 w-full bg-black/60 rounded border border-[#222] p-2 relative">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                ...
                <XAxis type="number" dataKey="dx" name="Tilt XY" unit="°" stroke="#555" fontSize={9} />
                <YAxis type="number" dataKey="dy" name="Strain Z" unit="mm" stroke="#555" fontSize={9} />
                <ZAxis type="category" dataKey="name" name="Location" />
                <Tooltip 
                ...
                <DebugDump data={vectors} />
<Scatter name="10th Floor Facade Drift" data={forceMapData(vectors)} fill="#00E5FF">
                  {vectors.map((entry, index) => {
                    const isHazardous = entry.dx > 10.0 || entry.dy > 10.0;
                    return <Cell key={`cell-${index}`} fill={isHazardous ? '#EF5350' : '#00E5FF'} opacity={0.8} />;
                  })}
                </Scatter>
              </ScatterChart>
            </ResponsiveContainer>
