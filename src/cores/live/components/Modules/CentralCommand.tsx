...
export default function CentralCommand() {
  ...
  // Customizable Early Warning Thresholds & Responses
  const [seismicThreshold, setSeismicThreshold] = useState(0.35); // in g-force
  const [fluidInfluxThreshold, setFluidInfluxThreshold] = useState(500); // m³/day
  const [mudPressureThreshold, setMudPressureThreshold] = useState(3620); // PSI
  const [emergencySupervisor, setEmergencySupervisor] = useState("Administrator");
  const [alertTemplate, setAlertTemplate] = useState("[EARLY WARNING] Critical telemetry deviance detected on Sumner Stratum. Arming automatic hydraulic cutoff.");
  const [autoCutoff, setAutoCutoff] = useState(true);
  const [autoVent, setAutoVent] = useState(true);
  const [showConfigDesk, setShowConfigDesk] = useState(true);

  // New WhatsApp contact list and dispatch handler
  const [whatsappContacts, setWhatsappContacts] = useState<any[]>([]);
  const [selectedContactNumber, setSelectedContactNumber] = useState("6285260245100");
    ...
    fetch('/api/whatsapp/contacts')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          if (data.length > 0) {
            setSelectedContactNumber(data[0].number);
          }
        } else if (data && Array.isArray(data.contacts)) {
          setWhatsappContacts(data.contacts);
          if (data.contacts.length > 0) {
            setSelectedContactNumber(data.contacts[0].number);
          }
        }
      })
      .catch(err => {
        ...
        const fallback = [
          { name: 'Ivan (Kordinator Utama)', number: '6285260245100', role: 'Geophysicist' },
    ...
    try {
      ...
      if (response.ok && data.success) {
        addLog({
          type: "INFO",
          source: "DISPATCHER",
          message: `📨 [DISPATCH SUCCESS] Emergency broadcast routed via Van-Botz to ${selectedContactNumber}. Status: SENT.`
        });
        alert(`Broadcast sent successfully to ${selectedContactNumber}!`);
      } else {
        throw new Error(data.error || "Server rejected dispatch");
      }
    } catch (err: any) {
      console.error(err);
      addLog({
        type: "WARN",
        source: "DISPATCHER",
        message: `❌ [DISPATCH FAILED] Direct WhatsApp alert transmission failed: ${err.message}`
      });
  ...
  // Auto-monitor mud pressure breach against custom threshold
  useEffect(() => {
    if (drillActive) {
      const currentPressure = 3500 + Math.sin(drillDepth / 20) * 150;
      if (currentPressure > mudPressureThreshold) {
        if (!emergencyActive) {
          toggleSiren();
          addLog({
            type: "WARN",
            source: "AUTO-ALARM",
            message: `🚨 [ALERT BREACH] Mud pressure of ${currentPressure.toFixed(0)} PSI exceeded threshold (${mudPressureThreshold} PSI). Dispatching early warning action to ${emergencySupervisor}.`
          });
        }
        if (autoCutoff) {
          ...
          addLog({
            ...
            message: `🛑 [SHUTOFF ENGAGED] Auto Mud Inject Cutoff activated. Driller halted at depth ${drillDepth.toFixed(1)}m. Supervisor ${emergencySupervisor} alerted: "${alertTemplate}"`
          });
        }
      }
    }
  }, [drillActive, drillDepth, mudPressureThreshold, autoCutoff, emergencySupervisor, emergencyActive, alertTemplate]);
...
Output a highly concise 3-sentence expert explanation.`;
  ...
  return (
    <div className={`flex flex-col h-full text-white bg-[#0A0A0B] p-6 space-y-6 overflow-y-auto scrollbar-thin ${emergencyActive ? "shadow-[inset_0_0_20px_rgba(239,68,68,0.15)]" : ""}`}>
      {emergencyActive && (
      ...
      <div className="flex items-center justify-between mb-2 border-b border-[#222] pb-4">
        ...
        <div className="flex gap-3">
          ...
          <button 
            onClick={() => {
              if (window.confirm("WARNING: Initiating Tactical Lockdown will instantly wipe all active sessions and local credentials. Proceed?")) {
                localStorage.clear();
                window.location.reload();
              }
            }}
            className="flex items-center gap-2 px-4 py-2 border border-rose-500/40 bg-rose-500/10 hover:bg-rose-500/30 text-rose-400 text-xs font-bold uppercase rounded-lg transition-all font-mono tracking-tight cursor-pointer shadow-[0_0_12px_rgba(244,63,94,0.15)]"
            title="TACTICAL LOCKDOWN: Wipes session and localStorage, securely locking the terminal"
          >
            <ShieldAlert size={14} /> TACTICAL LOCKDOWN
          </button>
      ...
      {/* EARLY WARNING RESPONSE SUITE */}
      <div className="bg-[#111116] border border-zinc-850 rounded-xl p-6 shadow-xl space-y-4 animate-fade-in" id="early-warning-customization">
        <div className="flex justify-between items-center border-b border-[#222] pb-3">
          <div className="flex items-center gap-2">
            <ShieldAlert className="text-red-500 animate-pulse" size={18} />
            <div>
              <h3 className="text-sm font-bold uppercase tracking-wider text-white">Early Warning Response Suite</h3>
              <p className="text-[10px] font-mono text-gray-500">Kustomisasi Parameter Telemetri & Tindakan Mitigasi Lapangan</p>
            </div>
          </div>
          <button 
            type="button"
            onClick={() => setShowConfigDesk(!showConfigDesk)}
            className="text-[9px] font-mono border border-zinc-800 px-2.5 py-1 rounded bg-[#16161a] text-[#888] hover:text-white transition-all cursor-pointer"
          >
            {showConfigDesk ? "HIDE DESK" : "SHOW DESK"}
          </button>
        </div>

        {showConfigDesk && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-xs font-mono">
            {/* Column 1: Threshold Adjustments */}
            <div className="space-y-4 bg-black/30 border border-[#222] p-4 rounded-lg">
              <span className="text-[10px] font-bold text-[#FF5722] uppercase tracking-wider block mb-2">Adjustable Tactical Thresholds</span>
              
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between mb-1 text-[9px] text-gray-400">
                    <span>Seismic Shaking Limit (PGA):</span>
                    <span className="text-white font-bold">{seismicThreshold.toFixed(2)}g</span>
                  </div>
                  <input 
                    type="range" min="0.05" max="1.0" step="0.05"
                    value={seismicThreshold} onChange={(e) => setSeismicThreshold(parseFloat(e.target.value))}
                    className="w-full accent-red-500 bg-[#222] h-1 rounded-full appearance-none outline-none cursor-pointer"
                  />
                  <span className="text-[8px] text-zinc-600 block mt-0.5">Triggers sirens above Mercalli Intensity VII</span>
                </div>

                <div>
                  <div className="flex justify-between mb-1 text-[9px] text-gray-400">
                    <span>Aquifer Flow Inflow Limit:</span>
                    <span className="text-white font-bold">{fluidInfluxThreshold} m³/day</span>
                  </div>
                  <input 
                    type="range" min="50" max="3000" step="50"
                    value={fluidInfluxThreshold} onChange={(e) => setFluidInfluxThreshold(parseInt(e.target.value))}
                    className="w-full accent-blue-500 bg-[#222] h-1 rounded-full appearance-none outline-none cursor-pointer"
                  />
                  <span className="text-[8px] text-zinc-600 block mt-0.5">Radial inflow reservoir flooding cutoff</span>
                </div>

                <div>
                  <div className="flex justify-between mb-1 text-[9px] text-gray-400">
                    <span>Borehole Mud Pressure Limit:</span>
                    <span className="text-white font-bold">{mudPressureThreshold} PSI</span>
                  </div>
                  <input 
                    type="range" min="3400" max="4000" step="10"
                    value={mudPressureThreshold} onChange={(e) => setMudPressureThreshold(parseInt(e.target.value))}
                    className="w-full accent-amber-500 bg-[#222] h-1 rounded-full appearance-none outline-none cursor-pointer"
                  />
                  <span className="text-[8px] text-zinc-600 block mt-0.5">Adjust lower to test the auto-shutdown loop!</span>
                </div>
              </div>
            </div>

            {/* Column 2: Mitigation Automation Actions */}
            <div className="space-y-4 bg-black/30 border border-[#222] p-4 rounded-lg">
              <span className="text-[10px] font-bold text-[#00E5FF] uppercase tracking-wider block mb-2">Automated Field Actions</span>
              
              {userRole !== 'ADMIN' && (
                <div className="bg-rose-500/10 border border-rose-500/30 text-rose-400 p-2 text-[8px] rounded mb-2 flex items-center gap-1.5">
                  <ShieldAlert className="w-3 h-3 shrink-0" />
                  RESTRICTED TO System Administrator LEVEL ONLY
                </div>
              )}

              <div className={`space-y-3.5 pt-1 ${userRole !== 'ADMIN' ? 'opacity-50 pointer-events-none' : ''}`}>
                <label className="flex items-center justify-between cursor-pointer group">
                  <div className="flex flex-col">
                    <span className="text-white text-[10px] font-bold">Auto Mud Inject Shutoff</span>
                    <span className="text-[8px] text-zinc-500">Halts borehole driller immediately if pressure limit is breached</span>
                  </div>
                  <input 
                    type="checkbox" checked={autoCutoff} onChange={(e) => setAutoCutoff(e.target.checked)}
                    className="w-3.5 h-3.5 accent-[#00E5FF] bg-[#1a1a1c] border-[#333] rounded cursor-pointer"
                ...
                <label className="flex items-center justify-between cursor-pointer group">
                  <div className="flex flex-col">
                    <span className="text-white text-[10px] font-bold">Vent Gas Release Open</span>
                    <span className="text-[8px] text-zinc-500">Vents hydrocarbon gases automatically on high pressure</span>
                  </div>
                  <input 
                    type="checkbox" checked={autoVent} onChange={(e) => setAutoVent(e.target.checked)}
                    className="w-3.5 h-3.5 accent-[#00E5FF] bg-[#1a1a1c] border-[#333] rounded cursor-pointer"
                    disabled={userRole !== 'ADMIN'}
                  />
                </label>
                ...
                <div className="pt-2 border-t border-zinc-900/80">
                  <span className="text-[8px] text-zinc-500 uppercase block mb-1">Active Relay Target</span>
                  <span className="text-emerald-400 font-bold text-[9px] flex items-center gap-1">
                    <span className="w-1 h-1 rounded-full bg-emerald-400 animate-pulse"></span>
                    SOLENOID-VALVE-RECEPTOR-PORT: 3000
                  </span>
                </div>
              </div>
            </div>

            {/* Column 3: Custom Alert Dispatch Parameters */}
            <div className="space-y-4 bg-black/30 border border-[#222] p-4 rounded-lg flex flex-col justify-between">
              <div>
                <span className="text-[10px] font-bold text-[#B554FF] uppercase tracking-wider block mb-2.5">Alert Dispatch Parameters</span>
                
                <div className="space-y-2.5">
                  <div className="flex flex-col gap-1">
                    <span className="text-[8px] text-zinc-500 uppercase">Emergency Safety Supervisor:</span>
                    <input 
                      type="text" 
                      value={emergencySupervisor} 
                      onChange={(e) => setEmergencySupervisor(e.target.value)}
                      className="bg-[#16161a] border border-[#222] px-2.5 py-1 text-[10px] text-white rounded focus:outline-none focus:border-[#B554FF]"
                    />
                  </div>

                  <div className="flex flex-col gap-1">
                    <span className="text-[8px] text-zinc-500 uppercase">Early Warning Broadcast SMS Template:</span>
                    <textarea 
                      value={alertTemplate} 
                      onChange={(e) => setAlertTemplate(e.target.value)}
                      className="bg-[#16161a] border border-[#222] p-2 text-[9px] text-gray-300 rounded focus:outline-none focus:border-[#B554FF] h-14 resize-none scrollbar-none"
                    />
                  </div>

                  <div className="pt-2 border-t border-zinc-900/40 space-y-2">
                    <div className="flex flex-col gap-1">
                      <span className="text-[8px] text-zinc-500 uppercase">Select Dispatch Contact:</span>
                      <select
                        value={selectedContactNumber}
                        onChange={(e) => setSelectedContactNumber(e.target.value)}
                        className="bg-[#16161a] border border-[#222] px-2 py-1 text-[10px] text-white rounded focus:outline-none focus:border-[#B554FF] outline-none cursor-pointer"
                      >
                        {whatsappContacts.map((contact, idx) => (
                          <option key={idx} value={contact.number} className="bg-[#16161a] text-white">
                    ...
                    <button
                      ...
                      className="w-full bg-[#B554FF]/20 hover:bg-[#B554FF]/30 active:bg-[#B554FF]/40 text-[#D19FFF] border border-[#B554FF]/40 hover:border-[#B554FF]/80 text-[9px] font-bold font-mono py-1.5 rounded transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                    >
                      {isSendingAlert ? (
                        <>
                          <span className="w-1.5 h-1.5 rounded-full bg-[#D19FFF] animate-ping"></span>
                          TRANSMITTING...
                        </>
                      ) : (
                        <>
