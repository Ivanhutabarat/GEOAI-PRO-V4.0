...
export default function OmniScienceWidget() {
  const baseFormulas: FormulaDef[] = [
    ...
    {
      id: 'barton_q_system',
      name: "Barton Q-System Tunnel & Wellbore Stability",
      category: 'Omega Expansion - Geotech',
      description: "Classifies rock mass stability and structural tunneling supports using joints & stresses.",
      inputs: [
        { key: 'rqd', label: 'Rock Quality Designation (RQD)', min: 5, max: 100, defaultValue: 75, step: 1, unit: '%', description: 'Percentage of intact rock core segments > 10cm.' },
        { key: 'jn', label: 'Joint Set Number (Jn)', min: 0.5, max: 20, defaultValue: 9, step: 0.5, unit: 'index', description: 'Number of joint planes and crack sets intersecting strata.' },
        { key: 'jr', label: 'Joint Roughness (Jr)', min: 0.5, max: 4.0, defaultValue: 1.5, step: 0.1, unit: 'index', description: 'Roughness scale of joint surfaces.' },
        { key: 'ja', label: 'Joint Alteration (Ja)', min: 0.75, max: 20, defaultValue: 1.0, step: 0.25, unit: 'index', description: 'Mineral infilling & weathering quality score.' },
        { key: 'jw', label: 'Joint Water Factor (Jw)', min: 0.05, max: 1.0, defaultValue: 1.0, step: 0.05, unit: 'factor', description: 'Water pressure inflow reduction factor.' },
        { key: 'srf', label: 'Stress Reduction Factor (SRF)', min: 0.5, max: 400, defaultValue: 1.0, step: 0.5, unit: 'scale', description: 'Loosening or squeezing load factor.' }
      ],
      calculate: (p) => OmniEngine.calculateBartonQSystem(p.rqd, p.jn, p.jr, p.ja, p.jw, p.srf)
    },
    {
      id: 'earthquake_pga',
      name: "Earthquake PGA Attenuation Model",
      category: 'Omega Expansion - Seismic',
      description: "Estimates expected peak ground acceleration forces decays over distances.",
      inputs: [
        { key: 'magnitude', label: 'Moment Magnitude (Mw)', min: 2.0, max: 9.5, defaultValue: 6.5, step: 0.1, unit: 'Mw', description: 'Richter seismic energy scale magnitude.' },
        { key: 'distanceKm', label: 'Epicenter Distance', min: 1.0, max: 500, defaultValue: 45, step: 5, unit: 'km', description: 'Radial distance to earthquake core rupture epicenter.' }
      ],
      calculate: (p) => OmniEngine.calculateEarthquakePGA(p.magnitude, p.distanceKm)
    },
    {
      id: 'darcy_radial_inflow',
      name: "Darcy Radial Wellbore Reservoir Inflow",
      category: 'Omega Expansion - Fluids',
      description: "Calculates cylindrical hydraulic inflow rates to boreholes from saturated sands.",
      inputs: [
        { key: 'k', label: 'Permeability (k)', min: 0.01, max: 100, defaultValue: 5.2, step: 0.5, unit: 'm/day', description: 'Strata hydraulic conduction coefficient.' },
        { key: 'H', label: 'Reservoir Thickness (H)', min: 1, max: 150, defaultValue: 30, step: 1, unit: 'meters', description: 'Vertical thickness of productive oil/water sand layer.' },
        { key: 'drawdown', label: 'Extraction Drawdown (Δh)', min: 0.5, max: 50, defaultValue: 8.5, step: 0.5, unit: 'meters', description: 'Hydraulic pressure head difference.' },
        { key: 'radiusRatio', label: 'Influence Radius Ratio (R/r)', min: 5, max: 5000, defaultValue: 1000, step: 50, unit: 'R/r', description: 'Ratio of external drainage radius to wellbore.' }
      ],
      calculate: (p) => OmniEngine.calculateDarcyRadialInflow(p.k, p.H, p.drawdown, p.radiusRatio)
    }
  ];
