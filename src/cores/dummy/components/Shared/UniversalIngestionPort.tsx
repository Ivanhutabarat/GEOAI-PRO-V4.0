...
import { parseLAS, parseSEGY, parseKML, parseGeoTIFF } from '../../../../lib/parsers';
...
export default function UniversalIngestionPort({
  ...
  const detectAndRoute = (fileName: string, headers: string[], rowCount: number, colCount: number) => {
    ...
    const isWellLog = lowerName.endsWith('.las') || joinedHeaders.includes('depth_m') || joinedHeaders.includes('gr_api') || joinedHeaders.includes('dept') || joinedHeaders.includes('depth');
    const isSeismic = lowerName.endsWith('.segy') || lowerName.endsWith('.sgy') || joinedHeaders.includes('time_ms') || joinedHeaders.includes('amplitude') || joinedHeaders.includes('trace_1');
    const isSpatial = lowerName.endsWith('.shp') || lowerName.endsWith('.tiff') || lowerName.endsWith('.tif') || lowerName.endsWith('.kml') || (joinedHeaders.includes('grid_x') && joinedHeaders.includes('elevation_z'));
    ...
    if (isWellLog) {
      ...
      if (!lowerName.endsWith('.segy') && !lowerName.endsWith('.sgy') && !lowerName.endsWith('.csv') && !lowerName.endsWith('.txt')) {
      ...
      if (!lowerName.endsWith('.shp') && !lowerName.endsWith('.tiff') && !lowerName.endsWith('.tif') && !lowerName.endsWith('.kml') && !lowerName.endsWith('.csv') && !lowerName.endsWith('.txt')) {
  ...
  const processAndTransmit = (text: string, fileName: string = "PASTED_DATA.txt") => {
    ...
    if (lowerName.endsWith('.las')) {
      try {
        ...
        parsedData = parsedLAS.data;
        headers = parsedLAS.curves;
        addLog({ type: 'INFO', source: 'Ingestion Engine', message: `Successfully parsed industry standard LAS file with ${parsedData.length} records.` });
      } catch (err) {
        console.error('[PARSER] LAS parsing failed:', err);
        addLog({ type: 'ERROR', source: 'Ingestion Engine', message: `LAS file parsing failed. Error: ${String(err)}` });
      }
    } else if (lowerName.endsWith('.segy') || lowerName.endsWith('.sgy')) {
      try {
        const parsedSegy = parseSEGY(text);
        parsedData = parsedSegy.chartData;
        headers = parsedSegy.traces.map(t => `trace_${t.traceNumber}`);
        addLog({ type: 'INFO', source: 'Ingestion Engine', message: `Successfully parsed industry standard SEG-Y textual/ASCII representation with ${parsedSegy.traces.length} traces.` });
      } catch (err) {
        console.error('[PARSER] SEG-Y parsing failed:', err);
        addLog({ type: 'ERROR', source: 'Ingestion Engine', message: `SEG-Y file parsing failed. Error: ${String(err)}` });
      }
    } else if (lowerName.endsWith('.kml')) {
      try {
        const parsedKml = parseKML(text);
        parsedData = parsedKml.map((placemark, index) => ({
          x: placemark.coordinates[0]?.lng || 0,
          y: placemark.coordinates[0]?.lat || 0,
          z: placemark.coordinates[0]?.elev || 0,
          color: placemark.type === 'Point' ? '#00E5FF' : '#FF5722',
          name: placemark.name,
          description: placemark.description
        }));
        headers = ['x', 'y', 'z', 'color', 'name'];
        addLog({ type: 'INFO', source: 'Ingestion Engine', message: `Successfully parsed industry standard KML map file with ${parsedData.length} features.` });
      } catch (err) {
        console.error('[PARSER] KML parsing failed:', err);
        addLog({ type: 'ERROR', source: 'Ingestion Engine', message: `KML file parsing failed. Error: ${String(err)}` });
      }
    } else if (lowerName.endsWith('.tiff') || lowerName.endsWith('.tif')) {
      try {
        const parsedGeo = parseGeoTIFF(text);
        const rows: any[] = [];
        parsedGeo.grid.forEach((row, rIdx) => {
        ...
        parsedData = rows;
        headers = ['grid_x', 'grid_y', 'elevation_z'];
        addLog({ type: 'INFO', source: 'Ingestion Engine', message: `Successfully parsed industry standard GeoTIFF/Grid with dimensions: ${parsedGeo.width}x${parsedGeo.height}` });
      } catch (err) {
        console.error('[PARSER] GeoTIFF parsing failed:', err);
        addLog({ type: 'ERROR', source: 'Ingestion Engine', message: `GeoTIFF file parsing failed. Error: ${String(err)}` });
      }
    }
    ...
    if (parsedData.length === 0) {
      ...
      const res = parseDataInput(finalPayloadText);
    ...
    <div className="geo-card border border-[#333] hover:border-[#FF5722]/40 transition-colors" id="universal-ingestion-card">
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2 items-center">
          ...
          >
            <Upload size={12} />
            Upload (.csv, .las, .segy, .kml, .tiff)
            <input 
              type="file" 
              accept=".txt,.csv,.las,.segy,.sgy,.tiff,.tif,.kml" 
