      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-7 bg-black/40 border border-[#333333] rounded-lg p-5 flex flex-col justify-between space-y-4">
          ...
          <div className="h-44 w-full bg-black/60 rounded border border-[#222] p-2">
            <DebugDump data={simulationTimeline} />
<ResponsiveContainer width="100%" height="100%">
              <LineChart data={forceMapData(simulationTimeline || [])}>
                <CartesianGrid strokeDasharray="3 3" stroke="#222" />
                <XAxis dataKey="time" stroke="#555" fontSize={9} tickLine={false} label={{ value: 'Time', position: 'insideBottom', offset: -2, fill: '#555', fontSize: 8 }} />
                <YAxis yAxisId="left" stroke="#00E5FF" fontSize={9} tickLine={false} label={{ value: 'Piezometric Level (m)', angle: -90, position: 'insideLeft', fill: '#00E5FF', fontSize: 8 }}/>
                <YAxis yAxisId="right" orientation="right" stroke="#FF5722" fontSize={9} tickLine={false} label={{ value: 'Pore Pressure (MPa)', angle: 90, position: 'insideRight', fill: '#FF5722', fontSize: 8 }}/>
                <Tooltip 
                  contentStyle={{ backgroundColor: '#111', borderColor: '#333', fontSize: 10, fontFamily: 'monospace' }}
                  labelStyle={{ color: '#fff' }}
                />
                <Line yAxisId="left" type="monotone" dataKey="level" name="Piezometric Level (m)" stroke="#00E5FF" strokeWidth={2} dot={{ r: 3 }} />
                <Line yAxisId="right" type="monotone" dataKey="porePressure" name="Pore Pressure (MPa)" stroke="#FF5722" strokeWidth={2} dot={{ r: 3 }} strokeDasharray="3 3" />
              </LineChart>
            </ResponsiveContainer>
          </div>
