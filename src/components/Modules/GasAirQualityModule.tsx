...
export default function GasAirQualityModule() {
  ...
  // Local Visualizer Timeline for Gas-Chromatography
  const [gasTimeline, setGasTimeline] = useState<any[]>([
    { elapsedMin: 0, H2S: 12, CH4: 2.1, CO: 8 },
    { elapsedMin: 2, H2S: 25, CH4: 2.2, CO: 10 },
    { elapsedMin: 4, H2S: 40, CH4: 3.5, CO: 15 },
    { elapsedMin: 6, H2S: 18, CH4: 3.1, CO: 12 },
    { elapsedMin: 8, H2S: 14, CH4: 2.6, CO: 9 },
  ]);
  ...
  const handleTransmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Update local emissions chart dataset
      setGasTimeline(prev => [
        ...prev,
        { elapsedMin: prev.length * 2, H2S: finalH2s, CH4: finalCh4, CO: finalCo }
      ].slice(-8));
          ...
          <div className="h-44 w-full bg-black/60 rounded border border-[#222] p-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={gasTimeline}>
                ...
                <XAxis dataKey="elapsedMin" stroke="#555" fontSize={9} tickLine={false} label={{ value: 'Survey Time (Min)', position: 'insideBottom', offset: -2, fill: '#555', fontSize: 8 }} />
