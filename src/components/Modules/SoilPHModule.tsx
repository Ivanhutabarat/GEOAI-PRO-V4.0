...
export default function SoilPHModule() {
  ...
  const [chemicalLevels, setChemicalLevels] = useState<any[]>([
    { chemical: 'Cadmium (Cd)', value: 1.2, limit: 1.5, color: '#00E676' },
    { chemical: 'Lead (Pb)', value: 45.0, limit: 50.0, color: '#00E676' },
    { chemical: 'Arsenic (As)', value: 12.5, limit: 10.0, color: '#FF5722' },
    { chemical: 'Mercury (Hg)', value: 0.3, limit: 1.0, color: '#00E676' },
  ]);
  ...
  const handleTransmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Update local chemical graph dynamics with user input
      setChemicalLevels(prev => {
        const copy = [...prev];
        copy[2] = { chemical: 'Arsenic (As)', value: finalMetal, limit: 10.0, color: finalMetal > 10.0 ? '#FF5722' : '#00E676' };
        return copy;
      });
        ...
        <div className="col-span-12 lg:col-span-7 bg-black/40 border border-[#333333] rounded-lg p-5 flex flex-col justify-between space-y-4">
          ...
          <div className="h-44 w-full bg-black/60 rounded border border-[#222] p-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chemicalLevels}>
                <CartesianGrid strokeDasharray="3 3" stroke="#222" />
                <XAxis dataKey="chemical" stroke="#555" fontSize={9} tickLine={false} />
                ...
                />
                <Bar dataKey="value" name="Detected Level (ppm)" fill="#00E676" radius={[4, 4, 0, 0]} />
                <Bar dataKey="limit" name="Maximum Safety Limit (ppm)" fill="#555" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
